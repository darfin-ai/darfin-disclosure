"""
지분공시(EQUITY, 대량보유상황보고서·임원ㆍ주요주주 소유보고서) 압축 + 요약/분석 프롬프트 모듈.

이 대분류는 "보유목적", "보유비율 변동", "변동사유"가 핵심인 짧고 정형화된 공시라서
rights_offering.py와 같은 항목형 압축 패턴을 그대로 따른다.
"""
import re
from dataclasses import dataclass

ITEM_LABELS: dict[str, list[str]] = {
    "EQ1_Holder_Info": ["보고자", "특별관계자", "본인과의 관계"],
    "EQ2_Holding_Purpose": ["보유목적", "단순투자", "경영참가", "일반투자"],
    "EQ3_Ratio_Change": ["보유주식등의 수", "보유비율", "직전 보고서", "변동후 보유비율"],
    "EQ4_Change_Reason": ["변동사유", "장내매수", "장내매도", "시간외매매", "신주인수"],
    "EQ5_Trading_Detail": ["거래일자", "거래단가", "취득단가", "처분단가"],
}
...
CRITICAL_KEYWORDS = [
    "경영참가", "경영권 영향", "최대주주 변경", "적대적 인수", "공개매수",
    "5% 이상 보유", "10% 이상 보유", "특별관계자 추가", "의결권 위임",
]
...
SUMMARY_SYSTEM_INSTRUCTION = (
    "당신은 국내 자본시장에서 대량보유상황보고서·임원 및 주요주주 소유보고서를 "
    "전문적으로 분석하는 애널리스트입니다. 보고자의 보유목적과 지분 변동 내역을 읽고, "
    "일반 투자자가 10초 만에 이 지분 변동이 경영권이나 주가에 미칠 영향을 이해할 수 있도록 "
    "직관적으로 요약해야 합니다. "
    "마크다운 기호나 부가 설명 없이 오직 지정된 JSON 객체 형식으로만 응답하십시오."
)

SUMMARY_USER_PROMPT_TEMPLATE = (
    "다음은 [{corp_name}]의 지분공시 핵심 항목을 발췌한 원문입니다.\n\n"
    "[요약 미션]\n"
    "제시된 원문을 바탕으로 이 지분 변동이 기존 주주와 경영권에 어떤 의미인지 "
    "총평하는 요약본을 작성하십시오.\n\n"
    "[Output Guidelines]\n"
    "1. summary_text: 보고자와 보유비율 변동폭을 1문장(50자 이내)으로 압축하십시오.\n"
    "2. investor_comment: 보유목적(단순투자/경영참가)과 변동사유를 근거로 향후 주가나 "
    "경영권에 미칠 영향을 3문장 이내로 해설하십시오.\n"
    "3. overall_risk: 경영참가 목적이거나 최대주주 변경 가능성이 있으면 'Critical' 또는 'High'를, "
    "단순투자 목적의 소폭 변동이면 'Low'나 'Neutral'을 부여하십시오.\n\n"
    "[JSON Schema]\n"
    "{{\n"
...
ANALYSIS_SYSTEM_INSTRUCTION = (
    "너는 지배구조 및 지분변동 전문 애널리스트다. 상장사의 '지분공시'를 분석하여, "
    "보유목적의 진위, 경영권에 미치는 영향, 지분 변동의 시장 시그널을 평가하라."
)

ANALYSIS_USER_PROMPT_TEMPLATE = (
    "[Mission]\n"
    "제공된 DART '지분공시' 발췌 구획을 분석하여, "
    "1) 보유목적의 성격(Holding_Purpose), 2) 경영권 영향(Governance_Impact), "
    "3) 지분 변동의 시장 시그널(Market_Signal)을 식별하고 JSON 배열로 반환하라.\n\n"
    "[Output Guidelines]\n"
    "1. 보유목적이 '경영참가'이거나 5% 이상 신규 보유가 발생하면 'Governance_Impact'에서 "
    "반드시 'High' 이상으로 분류하라.\n"
    ...
    "[\n"
    "  {{\n"
    '    "analysisCategory": "(Enum: [Holding_Purpose, Governance_Impact, Market_Signal])",\n'
    ...
    '    "materialImpact": "(String) 해당 지분 변동이 주가/경영권에 미칠 영향 분석 (3문장 이내)",\n'
