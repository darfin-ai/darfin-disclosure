"""
기타공시(OTHER, 자기주식취득·처분, 풍문또는보도에대한해명, 영업(잠정)실적 공시 등)
압축 + 요약/분석 프롬프트 모듈.

이 대분류는 형식이 가장 정형화되지 않은 영역이라(자기주식 신탁계약, 실적 잠정치 공시,
조회공시 답변 등 성격이 제각각), generic_disclosure.py보다는 구체적이지만
business_report.py만큼 고정된 섹션 구조를 가정할 수 없다. 그래서 항목형 라벨을
넓게 잡고, 매칭되는 라벨이 거의 없으면 GENERIC과 유사하게 앞부분을 폴백으로 쓴다.
"""
import re
from dataclasses import dataclass

ITEM_LABELS: dict[str, list[str]] = {
    "OT1_Decision_Summary": ["취득예정금액", "처분예정금액", "신탁계약금액", "조회공시 요구"],
    "OT2_Reason_Purpose": ["취득목적", "처분목적", "주가안정", "주주가치 제고"],
    "OT3_Schedule_Method": ["취득예정기간", "처분예정기간", "장내매수", "신탁계약기간"],
    "OT4_Performance_Figures": ["잠정 영업이익", "잠정 매출액", "잠정실적", "전년동기 대비"],
    "OT5_Response_Content": ["답변내용", "해명내용", "사실여부", "현재까지 확인된 내용"],
}
...
CRITICAL_KEYWORDS = [
    "전량 처분", "신탁계약 해지", "중대한 영향", "실적 급감", "어닝쇼크",
    "전년동기 대비 50%", "조회공시 부인", "확인 불가",
]
...
def compress_for_summary(full_text: str) -> str:
    ...
    if not parts:
        # 라벨이 하나도 안 잡히면(형식이 너무 다양함) 앞부분 + 숫자 포함 문장 위주로 폴백
        return _compress_fallback(full_text)
...
def _compress_fallback(text: str) -> str:
    front = text.strip()[:FALLBACK_MAX]
    return front
...
def extract_analysis_chunks(full_text: str) -> dict[str, ExtractedChunk]:
    ...
    if not result and full_text.strip():
        # 항목 라벨이 전혀 안 잡힌 경우 — 원문 앞부분 전체를 하나의 청크로 둔다.
        text = full_text[:FALLBACK_MAX]
        result["OT_FULL_TEXT"] = ExtractedChunk(text=text, start_offset=0)
...
SUMMARY_SYSTEM_INSTRUCTION = (
    "당신은 국내 자본시장에서 자기주식 취득/처분, 잠정실적, 조회공시 답변 등 "
    "다양한 형태의 기타공시를 전문적으로 분석하는 애널리스트입니다. "
    "공시 원문을 읽고, 일반 투자자가 10초 만에 핵심 내용과 주가에 미칠 영향을 "
    "이해할 수 있도록 직관적으로 요약해야 합니다. "
    "마크다운 기호나 부가 설명 없이 오직 지정된 JSON 객체 형식으로만 응답하십시오."
)

SUMMARY_USER_PROMPT_TEMPLATE = (
    "다음은 [{corp_name}]의 기타공시 핵심 내용을 발췌한 원문입니다.\n\n"
    "[요약 미션]\n"
    "제시된 원문을 바탕으로 이 공시가 기업가치와 주주에게 어떤 의미인지 "
    "총평하는 요약본을 작성하십시오.\n\n"
    "[Output Guidelines]\n"
    "1. summary_text: 공시의 핵심 내용을 1문장(50자 이내)으로 압축하십시오.\n"
    "2. investor_comment: 이 공시가 발생한 배경과 향후 주가 또는 실적에 미칠 영향을 "
    "3문장 이내로 해설하십시오.\n"
    "3. overall_risk: 실적 급감, 자기주식 전량 처분, 조회공시 부인 등 부정적 신호가 "
    "있으면 'Critical' 또는 'High'를, 주주가치 제고 목적의 자사주 매입 등 긍정적 신호면 "
    "'Low'를, 중립적 사실 공시면 'Neutral'을 부여하십시오.\n\n"
    "[JSON Schema]\n"
    "{{\n"
...
ANALYSIS_SYSTEM_INSTRUCTION = (
    "너는 기업 공시 분석 전문 애널리스트다. 상장사가 제출한 기타공시(자기주식, 잠정실적, "
    "조회공시 답변 등)를 분석하여, 주가와 실적에 미치는 실질적 영향을 평가하라."
)

ANALYSIS_USER_PROMPT_TEMPLATE = (
    "[Mission]\n"
    "제공된 DART '기타공시' 발췌 구획을 분석하여, "
    "1) 핵심 사실(Key_Fact), 2) 실적/주가 영향(Market_Impact), "
    "3) 향후 추가 확인이 필요한 불확실성(Uncertainty)을 식별하고 JSON 배열로 반환하라.\n\n"
    "[Output Guidelines]\n"
    "1. 실적이 전년동기 대비 50% 이상 악화되었거나 조회공시를 부인하는 내용이면 "
    "'Critical'로 분류하라.\n"
    ...
    "[\n"
    "  {{\n"
    '    "analysisCategory": "(Enum: [Key_Fact, Market_Impact, Uncertainty])",\n'
    ...
    '    "materialImpact": "(String) 해당 내용이 주가/실적에 미칠 영향 분석 (3문장 이내)",\n'
